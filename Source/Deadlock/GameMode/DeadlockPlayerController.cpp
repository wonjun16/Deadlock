	...
	if (IsLocalPlayerController())
	{
		...
		if (IsValid(WidgetClass))
		{
			...
			if (IsValid(LobbyWidget))
			{
				...
				FString RoleString = UEnum::GetValueAsString(LocalRole);
				GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, RoleString);

				ENetRole LocalRole22 = GetRemoteRole();
				FString RoleString22 = UEnum::GetValueAsString(LocalRole22);
				GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, RoleString22);
...
void ADeadlockPlayerController::ClientSetInGame_Implementation()
{
	UGameInstance* GameInstance = UGameplayStatics::GetGameInstance(GetWorld());
	UDeadlockGameInstanceSubsystem* MySubsystem = GameInstance->GetSubsystem<UDeadlockGameInstanceSubsystem>();
